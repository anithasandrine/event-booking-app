...
app.get('/events/upcoming', (req, res) => {
  // Assuming you have a method to fetch upcoming events from the database
  Event.find({ startDate: { $gte: new Date() } })
    .then(events => {
      // Render a view to display the upcoming events
      res.render('upcoming_events', { events: events });
    })
    .catch(error => {
